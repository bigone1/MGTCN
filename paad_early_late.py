...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "paad-label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "paad-mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "paad-me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "paad-mi.csv"), delimiter=','))
    data_tensor_list = []
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/paad/all'
    num_class=2
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,208,step=4) for i in range(p["gcn_layer"])],
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            ...
            'lr_e':0.00030381076009725093,
            'lr_c':0.001391700303985604,
            'weight_decay':0.0005718988285349066,
            'gcn_drop':0.5
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.00030381076009725093,0.01,0.1]:
    #     params['lr_e']=lr
    # alljieguo=[]
    # for opn in ["Adadelta","Adagrad","Adam","RMSprop","SGD"]:
    #     params['optimizer_name']=opn
    alljieguo=[]
    a=[96,160,224,308,340,372]
    for opn in a:
