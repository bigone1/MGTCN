...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "stad-label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "stad-mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "stad-me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "stad-mi.csv"), delimiter=','))
    data_tensor_list = []
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/stad/all'
    num_class=2
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,348,step=4) for i in range(p["gcn_layer"])],
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            ...
            'dim_he_list':[56,60],
            'optimizer_name':"Adam",
            'lr_e':0.00019237276849601466,
            'lr_c':0.004475083328807981,
            'weight_decay':0.0009256887783545936,
            'gcn_drop':0.4
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.00019237276849601466,0.01,0.1]:
    #     params['lr_e']=lr
    # alljieguo=[]
    # for opn in ["Adadelta","Adagrad","Adam","RMSprop","SGD"]:
    #     params['optimizer_name']=opn
    alljieguo=[]
    a=[16,32,56,120,184,248]
    for opn in a:
        params['dim_he_list']=[opn,60]
        seed_torch()
        adj_list = gen_trte_adj_mat(data_list, params['adj_parameter'])
        for i,edge in enumerate(adj_list):
            if i==0:
