...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "lihc-label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "lihc-mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "lihc-me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "lihc-mi.csv"), delimiter=','))
    data_tensor_list = []
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/lihc/all'
    num_class=2
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,300,step=4) for i in range(p["gcn_layer"])],
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            'adj_parameter':2,
            'dim_he_list':[172,280,64],
            'optimizer_name':"Adam",
            'lr_e':0.005640605127386479,
            'lr_c':0.004834225531972684,
            'weight_decay':0.0007010004132520177,
            'gcn_drop':0.4
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.005640605127386479,0.01,0.1]:
    #     params['lr_e']=lr
    # alljieguo=[]
    # for opn in ["Adadelta","Adagrad","Adam","RMSprop","SGD"]:
    #     params['optimizer_name']=opn
    alljieguo=[]
    a=[64,96,128,172,204,236]
    for opn in a:
        params['dim_he_list']=[opn,280,64]
        seed_torch()
        adj_list = gen_trte_adj_mat(data_list, params['adj_parameter'])
        for i,edge in enumerate(adj_list):
            if i==0:
