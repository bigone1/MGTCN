...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "coad-label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "coad-mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "coad-me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "coad-mi.csv"), delimiter=','))
    data_tensor_list = []
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/coad/all'
    num_class=2
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,320,step=4) for i in range(p["gcn_layer"])],
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            ...
            'dim_he_list':[296],
            'optimizer_name':"Adam",
            'lr_e':0.003917913304347703,
            'lr_c':0.0034005406482331573,
            'weight_decay':0.00033869741695739504,
            'gcn_drop':0.4
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    #学习率
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.003917913304347703,0.01,0.1]:
    #     params['lr_e']=lr
    #不同adjparameter
    # alljieguo=[]
    ...
    #神经元数量
    alljieguo=[]
    a=[48,96,144,192,240,296,344,392]
    for opn in a:
