...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "mi.csv"), delimiter=','))
    data_tensor_list = []
...
def init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hc, sample_weight,gcn_dopout,num_edge=4):
    model_dict = {}
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = GTN(num_edge=num_edge,num_channels=1,w_in=dim_list[i],w_out=dim_he_list,num_class=num_class,num_layers=1,norm=True,gcn_drop=gcn_dopout,sample_weight=sample_weight)
    if num_view >= 2:
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/lx'
    num_class=7
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,432,step=4) for i in range(p["gcn_layer"])],
    ...
    for i,(trind,teind) in enumerate(skf.split(data_list[0],labels)):
        ...
        for epoch in range(num_epoch):
            ...
            with torch.no_grad():
                ...
                te.append(acc)
        # plt.plot(range(num_epoch),losstr,label='tr')
        # plt.plot(range(num_epoch),losste,label='te')
        # plt.savefig(f'l{i}.png')
        # plt.close()
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            ...
            'dim_he_list':[404],
            'optimizer_name':"Adam",
            'lr_e':0.00011330480855982261,
            'lr_c':0.004839248991144873,
            'weight_decay':0.0001944010135009649,
            'gcn_drop':0.5
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    params={
            ...
            'dim_he_list':[404],
            'optimizer_name':"Adam",
            'lr_e':0.00005,#0.00011330480855982261,
            'lr_c':0.004839248991144873,
            'weight_decay':0.0001944010135009649,
            'gcn_drop':0.5
        }
    ...
    #学习率
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.00011330480855982261,0.001,0.01]:
    #     params['lr_e']=lr
    #优化函数
    # alljieguo=[]
            ...
            for epoch in range(num_epoch):
                ...
                with torch.no_grad():
                    ...
                    if epoch==num_epoch-1:
                        ...
                        jieguo.append(acc)
                        te_proball,c = test_epoch(data_list, A, labels_tensor,sample_weight, np.array(range(data_list[0].shape[0])),model_dict,)
                        # jieguo.append(te_proball)
                        # save_model_dict(f'models/{i}',model_dict)
            # plt.plot(range(num_epoch),losstr,label='train loss')
            # plt.plot(range(num_epoch),losste,label='test loss')
            # plt.xlabel('Epoch')
            # plt.ylabel('Loss value')
            # plt.title(f'Fold {i} learning curve')
            # plt.legend()
            # plt.savefig(f'fold{i}.png',dpi=300)
            # plt.close()
            sizhetest.append(te[-1])
            jieguo.append(labels[teind])
            jieguo.append(labels)
        # f=open('lexing1a3f.pkl','wb')
...
if __name__=='__main__':
    ...
    # study.optimize(objective,n_trials=1000,gc_after_trial=True,n_jobs=8)
    # res=[]
