...
def prepare_trte_data(data_folder):
    labels = np.loadtxt(os.path.join(data_folder, "esca-label.csv"), delimiter=',')
    labels = labels.astype(int)
    data = []
    data.append(np.loadtxt(os.path.join(data_folder, "esca-mr.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "esca-me.csv"), delimiter=','))
    data.append(np.loadtxt(os.path.join(data_folder, "esca-mi.csv"), delimiter=','))
    data_tensor_list = []
...
def objective(trial):
    seed_torch()
    data_folder='/home/zhoulin/ucsc/esca/all'
    num_class=2
    num_view = 3
    ...
    params={
            ...
            'dim_he_list':[trial.suggest_int(f'dim_he_list_{i}',8,200) for i in range(p["gcn_layer"])],
...
def cal_feat_imp(fold, model_folder):
    ...
    params={
            ...
            'lr_e':0.004124390730576327,
            'lr_c':0.0037840507339886947,
            'weight_decay':0.00041638760491000956,
            'gcn_drop':0.4
        }
    data_list, labels = prepare_trte_data(data_folder)
    labels_tensor = torch.LongTensor(labels)
...
def objective1():
    ...
    #学习率
    # alljieguo=[]
    # for lr in [0.000001,0.00001,0.004124390730576327,0.01,0.1]:
    #     params['lr_e']=lr
    # alljieguo=[]
    # for opn in ["Adadelta","Adagrad","Adam","RMSprop","SGD"]:
    #     params['optimizer_name']=opn
    alljieguo=[]
    a=[32,64,96,128,160,192]
    for opn in a:
...
if __name__=='__main__':
    ...
    # # out.close()
    # f1=optuna.visualization.plot_param_importances(study=study,target=lambda t:t.values[0])
    # f1.write_image('a.png')
    # end=time.time()
